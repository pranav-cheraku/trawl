...
interface DemoProjectLayoutProps {
  projectName: string;
  children: ReactNode;
}
...
export default function DemoProjectLayout({
  ...
  return (
    <div className="mx-auto max-w-screen-2xl px-6 py-5">
      ...
      {/* Tabs */}
      <nav className="mt-4 flex gap-1 overflow-x-auto" aria-label="Project tabs">
        {tabs.map((tab) => {
          const href = `/demo/${tab.segment}`;
          const isActive =
